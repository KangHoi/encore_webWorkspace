...
<%--
 jstl--  choose
 		 when
 		 otherwise
 index페이지에서는 클라이언트가 6가지 서비스를 요청할 수 있다.
 이 때 인증과 권한에 따라 요청할 수 있는 서비스가 달라져야 한다.
 
 authentification(인증)
 로그인 한 사용자 - 권한(authorization)
 				 로그아웃 / 전체 회원 정보 보기 / 정보 수정
...
<div id="wrap">
	...
	<c:choose>
		<c:when test="${!empty vo}">
			...
			<a href="update.jsp">회원정보수정</a><p>
			<a href="front.do?command=logout">로그 아웃</a><p>
		</c:when>
		<c:otherwise>
